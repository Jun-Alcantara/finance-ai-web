...
import { createIncome, IncomeInput } from "@/services/incomes"
...
export function CreateIncomeDialog({ open, onOpenChange, onSuccess }: CreateIncomeDialogProps) {
  ...
  const fetchDependencies = async () => {
    try {
      const [sourcesRes, bankAccountsRes] = await Promise.all([
        getSourceOfIncomes({ per_page: 100 }), // Fetch all for dropdown
  ...
  const handleSubmit = async () => {
    if (!formData.source_of_income_id || !formData.bank_account_id || !formData.amount || !formData.date) {
      return // Basic validation
    }

    setLoading(true)
    try {
      await createIncome(formData as IncomeInput)
      onSuccess()
      onOpenChange(false)
      resetForm()
    } catch (error) {
      ...
      <DialogContent className="sm:max-w-[500px]">
        ...
        <div className="grid gap-4 py-4">
               ...
               <div className="pt-2">
                 ...
                 <Popover>
                  ...
                  <p className="text-[10px] text-muted-foreground mt-1">
                    Income entries will be created up to this date.
                  </p>
               </div>
            )}
          </div>
          ...
          <div className="flex items-center space-x-2">
            ...
            />
            <Label htmlFor="received">Mark as Received</Label>
          </div>
        </div>
