...
import { updateIncome, Income, IncomeUpdateInput } from "@/services/incomes"
import { getSourceOfIncomes, SourceOfIncome } from "@/services/source-of-incomes"
...
export function EditIncomeDialog({ open, onOpenChange, onSuccess, income }: EditIncomeDialogProps) {
  const [loading, setLoading] = useState(false)
  const [sourceOfIncomes, setSourceOfIncomes] = useState<SourceOfIncome[]>([])
  const [bankAccounts, setBankAccounts] = useState<BankAccount[]>([])
  
  const [formData, setFormData] = useState<Partial<IncomeUpdateInput>>({})
  ...
  const initializeForm = (income: Income) => {
    setFormData({
      source_of_income_id: income.source_of_income_id,
      ...
      recurring_type: income.recurring_type || undefined,
      recurring_day: income.recurring_day || undefined,
      recur_until: income.recur_until || undefined,
    })
    setDate(parseISO(income.date))
    setRecurUntilDate(income.recur_until ? parseISO(income.recur_until) : undefined)
    setApplyToFuture(false)
  }

  const fetchDependencies = async () => {
    try {
      const [sourcesRes, bankAccountsRes] = await Promise.all([
        getSourceOfIncomes({ per_page: 100 }),
        getBankAccounts({ per_page: 100 })
      ])
      setSourceOfIncomes(sourcesRes.data)
  ...
  const handleSubmit = async () => {
    if (!income || !formData.date) return
        ...
        <div className="grid gap-4 py-4">
          ...
          <div className="grid grid-cols-2 gap-4">
            <div className="space-y-2">
              ...
              <Select 
                value={formData.source_of_income_id}
                onValueChange={(val) => setFormData({ ...formData, source_of_income_id: val })}
              >
                <SelectTrigger>
          ...
          <div className="flex items-center space-x-2">
             <Switch
              ...
              onCheckedChange={(checked) => setFormData({ ...formData, received: checked })}
            />
            <Label htmlFor="edit-received">Mark as Received</Label>
          </div>
           ...
           <div className="space-y-4 border rounded-md p-4 bg-muted/20">
            <div className="flex items-center space-x-2">
              ...
              />
              <Label htmlFor="edit-recurring">Recurring Income</Label>
            </div>
            ...
            {formData.is_recurring && (
             ...
             {income?.is_recurring && income.recurring_group_id && (
                 <div className="flex items-center space-x-2 mt-4 pt-4 border-t border-dashed">
                    ...
                    <Label htmlFor="apply-future" className="text-sm font-normal text-muted-foreground">
                        Apply changes to all future recurring incomes?
                    </Label>
                 </div>
             )}
          </div>
